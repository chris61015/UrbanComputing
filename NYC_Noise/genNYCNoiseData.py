...
def main():
    filePath = os.path.join(os.getcwd(),'RawData','311_Service_Requests_from_2010_to_Present.csv')
    with open(filePath) as f:
        lines = f.readlines()
        row = [lines[0]]
        startTime = timeConverter('2013/07/01','%Y/%m/%d')
        endTime = timeConverter('2014/07/01','%Y/%m/%d')
        timeList = []
        s = set()
        for line in lines[1:]:
            temp = line.split(',')
